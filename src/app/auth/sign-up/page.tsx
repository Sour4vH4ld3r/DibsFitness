  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          // If email confirmations are enabled in Supabase, user will need to verify
          // The callback route already exists at /auth/callback
      ...
      // Otherwise, ask user to check email
        ...
        <div className="backdrop-blur-xl bg-white/80 rounded-2xl shadow-2xl p-8 border border-white/20">
          ...
          <form onSubmit={handleSignUp} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 px-4 rounded-lg font-medium hover:from-blue-700 hover:to-purple-700 transition-all transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
            >
          ...
          <p className="mt-8 text-center text-sm text-gray-600">
            ...
            <Link href="/auth/sign-in" className="font-medium text-blue-600 hover:text-blue-700 transition-colors">
              Sign in
            </Link>
          </p>
        </div>
